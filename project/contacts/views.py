...
def contact(request):
    if request.method == 'POST':
        ...
        # check if the user_name has made inequiry already
        if request.user.is_authenticated:
            user_id = request.user.id
            has_contacted = Contact.objects.all().filter(listing_id=listing_id, user_id=user_id)
            if has_contacted:
                messages.error(request,'You have already made an inequiry for this listing')
        ...
        send_mail(
            ...
            [realtor_email, 'olanrewajudolapomicheal@gmail.com'],
            fail_silently=False
        )
